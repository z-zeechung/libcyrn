...
function compareOffset(buf1, buf2, targetStart, sourceStart, targetEnd, sourceEnd) {
    ...
    const buf1Start = (buf1.byteOffset || 0) + targetStart;
    const buf2Start = (buf2.byteOffset || 0) + sourceStart;

    const minLength = Math.min(targetLength, sourceLength);
    
    const cmpResult = _natives.memcmp(
...
function fill(buffer, value, offset, end, encoding) {
    ...
    if (ArrayBuffer.isView(value)) {
        ...
        // return on 0, in case of infinite loop
        if (bytesToCopy === 0) {
            target.fill(0);
            return;
        }
